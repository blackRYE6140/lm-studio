...
.letter-head { display: block; }
.template-B .bloc-sig { text-align: right; }
.template-C .letter-head { display: grid; grid-template-columns: 1fr 1fr; gap: 18mm; align-items: start; margin-bottom: 6mm; }
.template-C .expediteur, .template-C .destinataire { margin: 0; width: auto; }
.template-C .destinataire { justify-self: stretch; }
.template-D .letter-head { display: grid; grid-template-columns: 1fr 1fr; gap: 18mm; align-items: start; margin-bottom: 6mm; }
.template-D .expediteur, .template-D .destinataire { margin: 0; width: auto; }
.template-D .destinataire { justify-self: stretch; }
.template-D .bloc-sig { text-align: right; }
...
export function renderLetterDoc(data, opts = {}) {
  ...
  const template = ["A", "B", "C", "D"].includes(d.template) ? d.template : "A";
  ...
  const expediteur = `<div class="expediteur">${escapeHtml(d.expediteur.nom)}<br>${escapeHtml(d.expediteur.adresse)}<br>${contactTel}${contactMail}${contactSite}${contactGithub}${contactLinkedin}</div>`;
  const destinataire = `<div class="destinataire">${lines(d.destinataire)}</div>`;
  const body = `
<div class="letter-head">${expediteur}${destinataire}</div>
